...
/**
 * This class impements the daycount convention "30/360 ISDA".
 * 
 * Convention:
 * If day(startDate) = 31, then day(startDate) is set to 30
 * If day(endDate) = 31 and day(startDate) IN (30,31), then day(endDate) is set to 30
 * 
 * daycount = (year(endDate) - year(startDate)) * 360 + (month(endDate) - month(startDate)) * 30 + day(endDate) - day(startDate)
 * 
 * @author peter
 */
public class D30_360_ISDA extends BaseDaycountConvention implements DaycountConvention {
    ...
    public double getNumerator(Date startDate, Date endDate) throws DaycountException {
        ...
        if((endDay > 30) && (startDay >= 30)) endDay = 30;
    ...
    @Override
    public double getDenominator(Date startDate, Date endDate) {
        return 360.0;
    }
