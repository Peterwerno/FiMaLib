 ...
 * 'true' if the first subnode is less than or equal to the second subnode, 
 * otherwise it returns 'false'.
 * 
 * For complex numbers, LessEquals compares the ABSOLUTE value of the numbers,
 * while for Double numbers, the normal less equals (including negative 
 * numbers) is applied
 * 
 * @author Peter Werno
 */
public class LessEquals extends Node {
    /**
    ...
    /**
     * Creates a new instance of LessEquals with both subnodes given
     * 
     * @param left (Node) the left node
     * @param right (Node) the right node
     */
    public LessEquals(Node left, Node right) {
        super(2);
    ...
    @Override
    public Node copy() {
        return new LessEquals(this.subNodes[0].copy(), this.subNodes[1].copy());
    }
